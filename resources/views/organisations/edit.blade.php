            ...
            <form method="POST" action="/organisations/{{$organisation->organisation_id}}" enctype="multipart/form-data" class="p-10 flex flex-col items-center gap-5">
                ...
                <div class="flex flex-col gap-5 md:w-full">
                    <div>
                        ...
                        <x-text-input type="text" name="organisation_name" class="mt-1 block w-full" placeholder="{{$organisation->organisation_name}}" value="{{$organisation->organisation_name}}"></x-text-input>
                    ...
                    <div>
                        ...
                        <x-text-input type="text" name="address" class="mt-1 block w-full" placeholder="" value="{{$organisation->address}}"></x-text-input>
                        <x-input-error class="mt-2" :messages="$errors->get('address')" />
                    </div> 
                    
                    <div>
                        <x-input-label for="email">Organisation Email</x-input-label>
                        <x-text-input type="email" name="email" class="mt-1 block w-full" placeholder="" value="{{$organisation->email}}"></x-text-input>
                        <x-input-error class="mt-2" :messages="$errors->get('email')" />
                    </div> 
                    
                    <div>
                        <x-input-label for="contact_number">Organisation Contact Number</x-input-label>
                        <x-text-input type="text" name="contact_number" class="mt-1 block w-full" placeholder="" value="{{$organisation->contact_number}}"></x-text-input>
                        <x-input-error class="mt-2" :messages="$errors->get('contact_number')" />
                    </div> 
        
                    <div>
                        <x-input-label for="description">Organisation Description</x-input-label>
                        <textarea onkeydown="limitTextArea(this, 200)" onkeyup="limitTextArea(this, 200)" name="description" id="description" rows="8" class="mt-1 block w-full border-gray-300 focus:border-greenButtons focus:ring-greenButtons rounded-lg shadow-md" placeholder="Organisation Description...">{{$organisation->description}}</textarea>
